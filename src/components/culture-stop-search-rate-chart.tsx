...
export function CultureStopSearchRateChart({ data }: Props) {
  return (
        ...
        >
          ...
          <Tooltip
            ...
            formatter={(value) => {
              ...
              return [formatRate(numericValue, 1_000), "Rate per 1,000"];
            }}
            labelFormatter={(_label, payload) => {
              const row = payload?.[0]?.payload as StopSearchMetricRow | undefined;

              if (!row) {
                return "";
              }

              return `${row.label}: ${formatNumber(row.count)} searches across ${formatNumber(row.population)} people`;
            }}
          />
